...
const baseUrl = "https://raw.githubusercontent.com/Ilham555x/Keyy/refs/heads/main/Hanzz";
...
const verifyLogin = async () => {
    try {
        ...
        console.error(chalk.red('Error verifying login:'), error);
        return null;
    }
};

const commandPw = async () => {
    const getPassword = await question(chalk.green.bold('Masukkan Password Script: \n'));
    const data = await verifyLogin();
    if (!data || data.toString() !== getPassword.toString()) {
        console.log(chalk.blue.bold("Password Script Tidak Valid!"));
        if (fs.existsSync("nazedev")) {
            await fs.promises.rm("nazedev", { recursive: true, force: true });
        }
        process.exit(1);
    } else {
        console.log(chalk.blue.bold("Password Script Berhasil Terverifikasi ✓\n"));
    }
};
...
async function startNazeBot() {
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Hanz Bot'
		}
	}
